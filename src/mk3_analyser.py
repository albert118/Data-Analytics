...
def EWMA(inputs, *args, **kwargs):
	"""
	** depreciated ** 
	** see pandas.series.Series function ewm for updates **
...
def equiwidth(data, k, *args, **kwargs):
	...
	if "-D" in args:
		...
		print("Data min {}\nData max {}".format(data.min(), data.max()))
		input()
...
def histogram(data, mu, sigma, *args, **kwargs):
	...
	bins:		if False, bins are auto made by plt.
	returns tuple:
	...
	# sanity check
	if len(data)<1:
		print("Check your data input length! Must be an Nx1 array like vector!")
		return
	...
	if "-F" in args and bins is True:
		binned_input, binned_ranges, patches = plt.hist(data, bins=bins, density=False,facecolor='gray')
		plt.ylabel("Frequency")
		if '-D' in args:
			print("Bins: ", bins[:10])

	elif bins is False and '-F' in args:
		binned_input, binned_ranges, patches = plt.hist(data, density=False,facecolor='gray')
		plt.ylabel("Frequency")
	else:
...
def auto_run_hists(data, data_name, test_range, *args, **kwargs):
	...
	elif type(data) is not type(np.ndarray):
		data = data.to_numpy()
	else:
		pass
	...
	# sanity checks
	if test_range[1] < test_range[0] and len(test_range)<= 2:
		print("Test range is incompatible!! Check that range satisfies:\n")
		print("\ttest_range[1] > test_range[0]")
		return False
	else:
		if "-D" in args:
			print("Test Range Valid...")
		pass
	...
	for i in range(test_range[0], test_range[1]):
		...
		if "equidepth" in kwargs:
			...
			if "-D" in args:
				print('Equi-Width method employed...')
				print("Edges: ", edges[:10])
...
@timeit
def test_auto_hists():
	fn = "C:\\Users\\Albert\\OneDrive - UTS\\UTS\\01.02\\[DAntcs] Data Analytics\\Ass2 - Analytics Data Set\\Excel Processing\\Intro Data Analysis.xlsx"
	sheet="Orig. Data"
